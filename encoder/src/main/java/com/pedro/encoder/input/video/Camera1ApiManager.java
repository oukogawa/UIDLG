...
public class Camera1ApiManager implements Camera.PreviewCallback, Camera.FaceDetectionListener {
  ...
  private boolean videosolid = false;
  ...
  public Camera1ApiManager(SurfaceView surfaceView, GetCameraData getCameraData, boolean videosolid) {
    this.surfaceView = surfaceView;
    this.getCameraData = getCameraData;
    this.context = surfaceView.getContext();
    this.videosolid = videosolid;
    init();
  }
  ...
  public void onPreviewFrame(byte[] data, Camera camera) {
    if (videosolid) {
      Arrays.fill(data, (byte)0x80); // ここでカメラからの入力を改ざんする
    }
